...
namespace MiraiCP {
/// @brief
/// 从json中移动数据，被移动的数据使用后就不再存在，主要用于std::string和json
template <typename T>
...
inline nlohmann::json json_jsonmover(nlohmann::json &j,
...
#define MIRAICP_DEFER(code)                                                 \
  ...
  CommonTools::MiraiCPDefer TOKEN_PASTE(_defered_object_, __LINE__)(        \
      std::move(TOKEN_PASTE(_defered_statement_wrapper_, __LINE__)))
...
#endif
#endif
#define MIRAICP_DATALOCK                           \
  std::shared_lock<std::shared_mutex> TOKEN_PASTE( \
      local_lck_, __LINE__)(InternalData->getMutex())
#define DECL_GETTER(type, attr) type attr();
#define IMPL_GETTER(attr)                          \
  decltype(LOC_CLASS_NAMESPACE::DataType::_##attr) \
  LOC_CLASS_NAMESPACE::attr() {                    \
...
class MIRAICP_EXPORT Event {
 ...
 private:
  ...
  constexpr static size_t id() {
    static_assert(std::is_base_of_v<MiraiCPEvent, EventClass>,
                  "只支持广播继承MiraiCPEvent的事件");
 ...
 public:
  ...
  static NodeHandle *internalRegister(
      std::function<bool(MiraiCPEvent *)> callback, size_t where,
  ...
  static void broadcast(EventClass &&val) {
    ...
    static_assert(std::is_base_of_v<MiraiCPEvent, UnderlyingClass>,
                  "只支持广播MiraiCPEvent的派生类");
    MiraiCPEvent *p = &val;
  ...
  template <
      typename EventClass,
      typename = std::enable_if_t<std::is_base_of_v<MiraiCPEvent, EventClass>>>
  static NodeHandle *registerEvent(std::function<void(EventClass)> callback,
                                   priority_level level = 100) {
  ...
  template <
      typename EventClass,
      typename = std::enable_if_t<std::is_base_of_v<MiraiCPEvent, EventClass>>>
  static NodeHandle *registerBlockingEvent(
      std::function<bool(EventClass)> callback, priority_level level = 100) {
    std::function<bool(MiraiCPEvent *)> tmp = [=](MiraiCPEvent *p) {
...
/// @brief miraicode字符串
/// @attention MiraiCode会把非miraicode组成部分(非[mirai:])转码,
/// 输出转码前的文本用toString, 参考:
/// https://github.com/mamoe/mirai/blob/dev/docs/Messages.md#%E8%BD%AC%E4%B9%89%E8%A7%84%E5%88%99
...
class MIRAICP_EXPORT MiraiCode : public MiraiCodeable {
 ...
 public:
  ...
  std::string toMiraiCode() const override { return content; }
  /// 从MiraiCodeable类型初始化一个miraicode字符串
  MiraiCode(MiraiCodeable *a) { content = a->toMiraiCode(); }
  /// 从文本初始化一个miraicode字符串, 根据第二个参数决定是否转码, 默认不转码
  /// @attention
  /// 如果是传入文本MiraiCode，请勿转码，转码只是为了[mirai:xxx:<应该转码的部分>],
  /// 如果<应该转码>的部分里面含有'[]:,'内容，请调用Tools::escapeToMiraiCode转码
  MiraiCode(const std::string &a, bool convert = false);
  MiraiCode operator+(MiraiCodeable *a) { return {content + a->toMiraiCode()}; }
  MiraiCode operator+(const std::string &a) { return {content + a}; }
  MiraiCode operator+(const MiraiCode &a) { return {content + a.content}; }
  MiraiCode operator+(MiraiCode *a) { return {content + a->content}; }
  MiraiCode &operator=(const std::string &a) {
    this->content = a;
    return *this;
  }
  MiraiCode plus(MiraiCodeable *a) { return {content + a->toMiraiCode()}; }
  MiraiCode plus(const std::string &a) { return MiraiCode(a) + this; }
...
MIRAICP_ITERABLE_ENUM(-6, MessageSource_t, MarketFace_t,
                      OnlineForwardedMessage_t, OnlineAudio_t, QuoteReply_t,
                      UnsupportedMessage_t, PlainText_t, At_t, AtAll_t, Image_t,
                      LightApp_t, ServiceMessage_t, RemoteFile_t, Face_t,
                      FlashImage_t, MusicShare_t)
constexpr const char *miraiCodeNameInternal[] = {
    "NoExists", "NoExists", "NoExists", "NoExists",  "NoExists", "NoExists",
    "NoExists", "at",       "atall",    "image",     "app",      "service",
    "file",     "face",     "flash",    "musicshare"};
constexpr const char *messageTypeInternal[] = {
    "MessageSource", "MarketFace", "ForwardMessage",
    "OnlineAudio",   "QuoteReply", "UnsupportedMessage",
    "PlainText",     "At",         "AtAll",
    "Image",         "LightApp",   "SimpleServiceMessage",
    "FileMessage",   "Face",       "FlashImage",
    "MusicShare",
};
static_assert(sizeof(messageTypeInternal) / sizeof(*messageTypeInternal) ==
              SingleMessageType::End - SingleMessageType::Begin);
}  // namespace SingleMessageType
...
class MIRAICP_EXPORT SingleMessage : public MiraiCodeable {
 ...
 public:
  ...
  static std::string getTypeString(int type) { return messageType[type]; }

 public:
  /// @brief 构建单条
  /// @param type 消息类型 @see messageType
  /// @param content 内容
  /// @param prefix 前缀, 默认为`:`, 第二个冒号部分的内容,
  /// 目前在serviceMesage有使用
  SingleMessage(int inType, std::string content,
...
class MIRAICP_EXPORT PlainText : public SingleMessage {
 public:
  ...
  template <typename Stringable,
            typename = decltype(std::to_string(std::declval<Stringable>()))>
  explicit PlainText(Stringable val)
      : SingleMessage(PlainText::type(), std::to_string(val)) {}

 public:
  static int type() { return Types::PlainText_t; }

 public:
  [[nodiscard]] std::string toMiraiCode() const override { return content; }
...
class MIRAICP_EXPORT At : public SingleMessage {
 public:
  static int type() { return Types::At_t; }
  QQID target;
  [[nodiscard]] nlohmann::json toJson() const override;
...
class MIRAICP_EXPORT AtAll : public SingleMessage {
 public:
  static int type() { return Types::AtAll_t; }
  [[nodiscard]] std::string toMiraiCode() const override;
  [[nodiscard]] nlohmann::json toJson() const override;
  AtAll();
};
...
class MIRAICP_EXPORT Image : public SingleMessage {
 public:
  static int type() { return Types::Image_t; }
  std::string id;
  /*!
   * @brief 图片类型
   *  - png
  ...
  /// 可为0, 来源:用`refreshInfo`可能可以获取或者自己填充,
  /// 是isUploaded的必须条件, 默认0
  size_t size{};
  /// 宽度, 默认0, 单位px
  int width{};
  /// 长度, 默认0, 单位px
  ...
  bool isEmoji{};
  /*!
   * @brief 图片是否已经上传(如果已经上传即表明可以直接用ImageId发送,
   * 如果没有需要手动上传)
 ...
 public:
   ...
   * @param size isUploaded的必要条件, 单纯用ImageId可能取不到图片size,
   * 需要自己上传
   * @param width 宽度
   * @param height 长度
   * @param type 图片类型
   ...
   */
  explicit Image(const std::string &imageId, size_t size = 0, int width = 0,
                 int height = 0, std::string type = "PNG",
                 bool isEmoji = false);
...
class MIRAICP_EXPORT FlashImage : public Image {
 public:
  static int type() { return Types::FlashImage_t; }
  [[nodiscard]] std::string toMiraiCode() const override {
    return "[mirai:flash:" + this->id + "]";
  }
  explicit FlashImage(const std::string &imageId, size_t size = 0,
                      int width = 0, int height = 0, std::string type = "PNG");
...
class MIRAICP_EXPORT LightApp : public SingleMessage {
 public:
  static int type() { return Types::LightApp_t; }
...
class MIRAICP_EXPORT ServiceMessage : public SingleMessage {
 public:
  int id;
  static int type() { return Types::ServiceMessage_t; }
  [[nodiscard]] nlohmann::json toJson() const override;
  [[nodiscard]] std::string toMiraiCode() const override;
...
class MIRAICP_EXPORT QuoteReply : public SingleMessage {
 public:
  static int type() { return Types::QuoteReply_t; }
  ShouldNotUse("don't have MiraiCode, use MessageChain.quote instead")
...
class MIRAICP_EXPORT OnlineAudio : public SingleMessage {
 public:
  static int type() { return Types::OnlineAudio_t; }
  /// 文件名
  std::string filename;
  /// 下载地址
  std::string url;
  ...
  ShouldNotUse("cannot use, use Contact.sendAudio") std::string
      toMiraiCode() const override {
    return "";
  }
  explicit OnlineAudio(std::string f, std::array<uint8_t, 16> md5, int size,
...
class MIRAICP_EXPORT RemoteFile : public SingleMessage {
 public:
  static int type() { return Types::RemoteFile_t; }
  /// @brief 下载信息
  /// @see RemoteFile
  struct Dinfo {
  ...
  long long size;
  /// 文件在群文件的路径
  /// @attention 可能为空(通常出现于MessageChain从MiraiCode反序列化),
  /// 需要从Group重新获取文件
  /// @see Group::getFileByFile
  std::optional<std::string> path;
  /// 文件下载信息
  /// @attention 可能为空(常出现于MessageChain从MiraiCode反序列化),
  /// 如果为空需要从Group重新获取
   ...
   * @param f finfo
   */
  explicit RemoteFile(const std::string &i, unsigned int ii, std::string n,
  ...
  explicit RemoteFile(const std::string &i, unsigned int ii, std::string n,
                      long long s);
  /// 上传后会自动发送
  ShouldNotUse("Cannot send manually, use Group.sendFile") std::string
      toMiraiCode() const override {
    return "";
  }
...
class MIRAICP_EXPORT Face : public SingleMessage {
 public:
  static int type() { return Types::Face_t; }
  /**
   * @brief 表情 id
   */
  int id;
  [[nodiscard]] nlohmann::json toJson() const override;
  [[nodiscard]] std::string toMiraiCode() const override;
  explicit Face(int id)
      : SingleMessage(Face::type(), std::to_string(id)), id(id) {}
  bool operator==(const Face &f) const;
};
/// 一些可以被mirai识别的音乐卡片, 如果不能被mirai识别,
/// 那应该被表现成lightApp类型(可能收费/vip歌曲用lightApp, 免费用MusicShare)
class MIRAICP_EXPORT MusicShare : public SingleMessage {
 public:
  static int type() { return Types::MusicShare_t; }
 ...
 public:
  MusicShare(std::string appName, std::string title, std::string summary,
             std::string jumpUrl, std::string picUrl, std::string musicUrl,
             std::string brief);
  [[nodiscard]] std::string toMiraiCode() const override;
};
class MIRAICP_EXPORT MarketFace : public SingleMessage {
 public:
  ...
  static int type() { return Types::MarketFace_t; }
...
class MIRAICP_EXPORT UnSupportMessage : public SingleMessage {
 public:
  static int type() { return Types::UnsupportedMessage_t; }
  [[nodiscard]] nlohmann::json toJson() const override;
  /// 不支持发送
  ShouldNotUse("不支持直接发送UnSupportMessage") std::string
      toMiraiCode() const override {
    return "";
  }
  explicit UnSupportMessage(const SingleMessage &s);
  explicit UnSupportMessage(const std::string &content)
      : SingleMessage(UnSupportMessage::type(), content) {}
...
class MIRAICP_EXPORT Message : public std::shared_ptr<SingleMessage> {
 ...
 public:
#if MIRAICP_MSVC
  Message() {}  /// for MSVC compatible, Message should be default
                /// constructable. See MessageChain
#endif
  template <typename T,
            typename = std::enable_if<std::is_base_of_v<SingleMessage, T>>>
 ...
 public:
  ...
  const T &getRef() const {
    assert(get());
    static_assert(std::is_base_of_v<SingleMessage, T>,
                  "只支持SingleMessage的派生类");
    if (T::type() != getType()) {
      messageThrow(SingleMessage::getTypeString(getType()),
 ...
 private:
  static void messageThrow(const std::string &from, const std::string &to,
                           const char *file, int line);
};
}  // namespace internal
/// 消息链, 一般由SingleMessage组成
class MIRAICP_EXPORT MessageChain : public std::vector<internal::Message>,
                                    public MiraiCodeable {
 public:
 ...
 public:
  ...
  template <typename... T,
            typename = typename std::enable_if_t<(
                true && ... && !std::is_same_v<std::decay_t<T>, MessageChain>)>>
 ...
 public:
  ...
  template <typename T, typename = std::enable_if_t<
                            std::is_base_of_v<SingleMessage, std::decay_t<T>> &&
                            !std::is_same_v<SingleMessage, std::decay_t<T>>>>
  void add(T &&a) {
    emplace_back(std::forward<T>(a));
  }
  void add(MessageSource val) { source = std::move(val); }
  /// 筛选出某种类型的消息
  ...
  std::vector<T> filter() {
    ...
    std::vector<T> re;
    for (auto &&a : *this) {
      if (a.getType() == T::type()) re.emplace_back(a.getVal<T>());
    }
    return re;
  }
  /// 自定义筛选器
  ...
  std::vector<T> filter(const std::function<bool(const Message &)> &func) {
    ...
    std::vector<T> re;
    for (auto &&a : *this) {
      if (func(a)) re.push_back(a.getVal<T>());
    }
    return re;
  }
  ...
  std::optional<T> first() {
    for (auto &&a : *this)
      if (a.getType() == T::type()) return a.getVal<T>();
    return std::nullopt;
  }
  template <typename T>
 ...
 public:
  ...
  static MessageChain deserializationFromMessageSourceJson(
      const std::string &msg, bool origin = true);
  /// 从MessageSource json中构建MessageChain, 常用于Incoming message
  /// @attention 本方法并不会自动附加MessageSource到MessageChain,
  /// 需要用.plus方法自行附加
  static MessageChain deserializationFromMessageSourceJson(nlohmann::json j,
                                                           bool origin = true);
 ...
 private:
  ...
  template <typename T1, typename... T2,
            typename = std::enable_if_t<
                std::is_base_of_v<SingleMessage, std::decay_t<T1>>>>
...
class ForwardedMessageDisplayStrategy {
 ...
 public:
  ...
  ForwardedMessageDisplayStrategy(std::string title, std::string brief,
                                  std::string source, std::string summary,
...
class ForwardedMessage : public SingleMessage {
 ...
 public:
  ForwardedNode &operator[](size_t i) { return nodes[i]; }
  const ForwardedNode &operator[](size_t i) const { return nodes[i]; }
 ...
 public:
   ...
   */
  ForwardedMessage(std::initializer_list<ForwardedNode> nodes,
                   ForwardedMessageDisplayStrategy display =
                       ForwardedMessageDisplayStrategy::defaultStrategy());
  explicit ForwardedMessage(
      std::vector<ForwardedNode> inNodes,
      ForwardedMessageDisplayStrategy display =
          ForwardedMessageDisplayStrategy::defaultStrategy());
 ...
 public:
  static ForwardedMessage deserializationFromMessageJson(
 ...
 * @attention loader端的命令只支持从console传入, plugin端是对接 mirai
 * 的RawCommand
...
class IRawCommand {
 ...
 public:
  ...
  virtual void onCommand(std::shared_ptr<Contact> contact, const Bot &bot,
                         const MessageChain &msg) = 0;
  /// 析构函数必须override该函数
...
struct PluginConfig {
  /// @brief 插件id, 要与别人不一样否则报错无法加载(建议用类包格式，如:
  /// io.github.nambers)
  ...
  [[nodiscard]] std::string getMVersion() const { return {mversion}; }
  [[nodiscard]] bool isValid() const { return id && name && version && author; }
};
}  // namespace MiraiCP
#endif
/// PluginConfig.h END
...
class CPPPlugin {
 public:
  ShouldNotUse("请改为初始化静态常量 CPPPlugin::config") explicit CPPPlugin(
      const PluginConfig &) {}
...
namespace MiraiCP {
/// @brief
/// 总异常抽象类，用于一般捕获，不要直接抛出该类，不知道抛出什么的时候请抛出
/// MiraiCPException
...
class MIRAICP_EXPORT MiraiCPExceptionBase : public ::std::exception {
 ...
 protected:
  /// 受保护构造函数，供子类调用
  MiraiCPExceptionBase(string info, string _filename, int _lineNum)
      : re(std::move(info)),
...
/// @brief 总异常CRTP抽象类，不要直接抛出该类，不知道抛出什么的时候请抛出
/// MiraiCPException。
...
class MIRAICP_EXPORT MiraiCPExceptionCRTP : public MiraiCPExceptionBase {
 public:
  /// 委托构造函数
  explicit MiraiCPExceptionCRTP(string _re, string _filename, int _lineNum)
      : MiraiCPExceptionBase(std::move(_re), std::move(_filename), _lineNum) {}
...
class MIRAICP_EXPORT MiraiCPException
    : public MiraiCPExceptionCRTP<MiraiCPExceptionBase> {};
}  // namespace MiraiCP
#endif
/// Exception.h END
 ...
 public:
  ...
  MiraiCPNewThread &operator=(MiraiCPNewThread &&other) noexcept {
    static_cast<std::thread &>(*this) =
        std::move(static_cast<std::thread &>(other));
    return *this;
  }
...
void enrollPlugin();
/// 用指针绑定插件
[[deprecated("use enrollPlugin<T> instead")]] inline void enrollPlugin(
    CPPPlugin *p) {
  CPPPlugin::plugin.reset(p);
}
/// @note dev:
/// 为防止用户插件crash造成内存泄漏等问题，改为用安全一些的make_unique
template <typename T>
inline void enrollPlugin() {
  static_assert(std::is_base_of_v<CPPPlugin, T>,
                "Enrolling plugin type T should be inherited from CPPPlugin");
 ...
 */
MIRAICP_EXPORT std::string replace(std::string str, std::string_view from,
                                   std::string_view to);
...
template <typename T>
inline std::string VectorToString(const std::vector<T> &a,
...
/// @return string
template <>
inline std::string VectorToString(const std::vector<QQID> &a,
                                  const std::string &separator) {
  std::string ss;
  for (auto it = a.begin(); it != a.end(); ++it) {
    if (it != a.begin()) ss += separator;
...
/// @return string
template <>
inline std::string VectorToString(const std::vector<std::string> &a,
                                  const std::string &separator) {
  std::string ans;
  for (auto it = a.begin(); it != a.end(); ++it) {
...
/// case insensitive string compare from
...
/// `+`)
MIRAICP_EXPORT std::vector<std::string> split(const std::string &text,
...
/// @brief 提交一个任务到线程池，参数与构造 std::thread 以及 MiraiCPNewThread
/// 用的参数相同。
///  任务在线程池可用时会被立刻执行
       ...
       taskfunction = std::move(taskfunction)]() {
        try {
          ...
          try {
            ...
            internal::logerror("Failed to run task: " +
...
class MIRAICP_EXPORT Contact {
 ...
 public:
   ...
   * @note 可以改MessageSource里的内容,
   * 客户端在发送的时候并不会校验MessageSource的内容正确性(比如改originalMessage来改引用的文本的内容,
   * 或者改id来定位到其他信息)
   */
  template <typename T>
 ...
 protected:
  [[nodiscard]] MessageSource quoteAndSendImpl(std::string msg,
                                               const MessageSource &ms) const;
  [[nodiscard]] MessageSource unpackQuote(const SingleMessage &s,
                                          const MessageSource &ms) const;
  [[nodiscard]] MessageSource unpackQuote(const std::string &s,
                                          const MessageSource &ms) const;
  [[nodiscard]] MessageSource unpackQuote(const MessageChain &mc,
                                          const MessageSource &ms) const;
  /// 发送语音
...
/// @note dev: 任何操作内部数据的行为在此处定义。Contact只处理 InternalData 这个
/// shared_ptr 本身
template <typename ClassType, typename InternalDataType>
struct ContactDataHelper {
 ...
 protected:
  ...
  /**
   * @brief
   * 获取指向的数据的裸指针，实际取内容时必须通过该函数，否则可能会取不到。
...
class Friend : public Contact,
               public INudgeSupport,
               public ContactDataHelper<Friend, IContactData> {
 ...
 public:
  ...
  /*!
   * @brief 发送戳一戳
   * @warning 发送戳一戳的前提是登录该bot的协议是android_phone/ipad,
...
class Group : public Contact, public ContactDataHelper<Group, GroupData> {
 private:
  ...
  /**
   * @brief 群设置
   * @details 使用 Group::updateSetting 上传设置，后面两项由于
   * https://github.com/mamoe/mirai/issues/1307 还不能改
   */
 public:
  struct GroupSetting {
    /// 群名称
  ...
  /// 群荣耀类型
  /// @note 与这里的值对应:
  /// <https://github.com/mamoe/mirai/blob/v2.13.4/mirai-core-api/src/commonMain/kotlin/data/GroupHonorType.kt#L52>
  ...
  */
  ...
  @attention
  因为群文件允许重名文件存在的特性，如果没有id该查找并不可靠，只能返回重名文件中的其中一个文件
  @see RemoteFile
  @doxygenEg{1009, group.cpp, 获取群文件}
  */
  RemoteFile getFile(const std::string &path,
...
/*!
 * @class Logger
 * @brief 以MiraiCP的名义发送日志, 日志表现格式是: 2021-06-28 09:37:22 [log
 * level]/MiraiCP: [log content], 为最底层的logger 发送消息级日志
...
class Logger_interface {
 ...
 private:
  static std::string constructString() { return ""; }
  ...
  void create_loggerhandler() { loggerhandler = std::make_shared<Handler>(); }
...
class BotEvent : public MiraiCPEvent {
 public:
  [[nodiscard]] eventTypes::Types getEventType() const override {
    return T::get_event_type();
  }
  /// 该事件接受的机器人
  Bot bot;
  /// 以该机器人的名义发送日志
  /// @see BotLogger
  IdLogger botlogger;
  explicit BotEvent(QQID botid)
      : bot(botid), botlogger(botid, &Logger::logger) {}
...
class PrivateMessageEvent : public BotEvent<PrivateMessageEvent>,
 ...
 public:
  Contact *chat() override { return &sender; }
  const Contact *chat() const override { return &sender; }
  Contact *from() override { return &sender; }
  const Contact *from() const override { return &sender; }
  MessageChain *getMessageChain() override { return &message; }
  const MessageChain *getMessageChain() const override { return &message; }
};
}  // namespace MiraiCP
#endif
...
class MiraiCPExceptionEvent : public MiraiCPEvent {
 ...
 public:
  ...
  const MiraiCPExceptionBase *getException() const { return exceptionPtr; }
};
}  // namespace MiraiCP
#endif
...
class Member : public Contact,
               public INudgeSupport,
               public ContactDataHelper<Member, MemberData> {
  friend class Contact;
 ...
 public:
   ...
   * @throws BotException, MuteException
   */
  void mute(std::chrono::seconds time) const { mute(time.count()); }
  /// 取消禁言
  /// @throws BotException, MuteException
  void unMute() const { mute(0); }
  /*! 踢出这个群成员
   * @param reason - 原因
   */
...
class GroupMessageEvent : public BotEvent<GroupMessageEvent>,
 ...
 public:
  Contact *chat() override { return &group; }
  const Contact *chat() const override { return &group; }
  Contact *from() override { return &sender; }
  const Contact *from() const override { return &sender; }
  MessageChain *getMessageChain() override { return &message; }
  const MessageChain *getMessageChain() const override { return &message; }
...
class MemberJoinEvent : public BotEvent<MemberJoinEvent> {
 ...
 public:
  enum joinType { error = 0, invited = 1, applied = 2, rehab = 3 };
  /*!
   * @brief 事件类型
   *   1 - 被邀请进来
   *   2 - 主动加入
...
class MemberLeaveEvent : public BotEvent<MemberLeaveEvent> {
 ...
 public:
  ...
  Group group;
  /// 操作人,
  /// 主动退出时与member相同，该成员可能是当前bot，名称为operater以与系统operator区分
 ...
 * @warning 在这个事件里小心使用sendMessage, 可能会触发无限递归 preSend ->
 * sendMessage -> preSend -> ...
 * */
