...
#include <string>
namespace MiraiCP {
void ErrorHandle0(const std::string &name, int line, const std::string &re,
...
class MIRAICP_EXPORT APIException : public MiraiCPExceptionCRTP<APIException> {
 public:
  explicit APIException(const std::string &text, string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("MiraiCP内部无法预料的错误:" + text,
...
/// 参数错误
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT IllegalArgumentException
    : public MiraiCPExceptionCRTP<IllegalArgumentException> {
 public:
  explicit IllegalArgumentException(const string &e, string _filename,
                                    int _lineNum)
      : MiraiCPExceptionCRTP(e, std::move(_filename), _lineNum) {}
 ...
 * @return 返回数据
 */
std::string ktOperation(operation_set type, const nlohmann::json &data,
                        bool catchErr = true,
 ...
 * @return 返回数据
 */
std::string ktOperationStr(operation_set type, const std::string &data,
                           bool catchErr = true,
                           const std::string &errorInfo = "");
}  // namespace MiraiCP::KtOperation
#endif
...
ForwardedNode::ForwardedNode(QQID id, std::string name,
                             ForwardedMessage _message, int t)
    : id(id),
      name(std::move(name)),
      message(MessageChain(std::move(_message))),
      time(t),
      isForwardedMessage(true) {}
ForwardedNode::ForwardedNode(QQID id, std::string name, MessageChain message,
                             int time)
    : id(id),
      name(std::move(name)),
...
bool ForwardedMessageDisplayStrategy::operator==(
    const ForwardedMessageDisplayStrategy &other) const {
...
ForwardedMessageDisplayStrategy ForwardedMessageDisplayStrategy::fromJson(
    const json &j) {
  return {j["title"], j["brief"], j["source"], j["summary"],
          j["preview"].get<std::vector<std::string>>()};
}
ForwardedMessageDisplayStrategy::ForwardedMessageDisplayStrategy(
    std::string title, std::string brief, std::string source,
    std::string summary, std::vector<std::string> preview)
...
bool ForwardedMessage::operator==(const ForwardedMessage &m) const {
  ...
  return std::all_of(
      ...
      [&i, &m](const auto &n) { return n.message == m[i++].message; });
}
...
MessageSource ForwardedMessage::sendTo(Contact *c) const {
  ...
  json req{{"contact", c->toJson()}, {"msg", json::array({msg}).dump()}};
  std::string re = KtOperation::ktOperation(KtOperation::Buildforward, req);
  MIRAICP_ERROR_HANDLE(re, "");
  return MessageSource::deserializeFromString(re);
}
ForwardedMessage ForwardedMessage::deserializationFromMessageJson(
    const json &msg) {
  if (!(msg.contains("type") && msg["type"] == "ForwardMessage") &&
      msg.contains("nodeList"))
    throw std::invalid_argument("json is not a ForwardMessage");
  ForwardedMessageDisplayStrategy display;
  if (msg.contains("title") && msg.contains("brief") &&
      msg.contains("source") && msg.contains("summary") &&
  ...
  auto &j = msg["nodeList"];
  if (!j.is_array())
    throw IllegalArgumentException("OnlineForwardedMessage参数异常, 应为 array",
                                   MIRAICP_EXCEPTION_WHERE);
  try {
    for (auto &&a : j) {
      if (a["messageChain"][0].contains("kind") &&
          a["messageChain"][0]["kind"] == "FORWARD") {
        nodes.emplace_back(a["senderId"], a["senderName"],
                           ForwardedMessage::deserializationFromMessageJson(
                               a["messageChain"][1]["nodeList"]),
                           a["time"]);
      } else {
        nodes.emplace_back(
            a["senderId"], a["senderName"],
            MessageChain::deserializationFromMessageJson(a["messageChain"]),
            a["time"]);
      }
    }
  } catch (const json::parse_error &) {
    throw APIException("OnlineForwardedMessage格式化异常",
                       MIRAICP_EXCEPTION_WHERE);
  }
  return ForwardedMessage(std::move(nodes), display);
}
nlohmann::json ForwardedMessage::toJson() const {
  return {{"type", "ForwardMessage"}, {"preview", display.preview},
          {"source", display.source}, {"title", display.title},
          {"brief", display.brief},   {"summary", display.summary},
...
ForwardedMessage::ForwardedMessage(std::initializer_list<ForwardedNode> nodes,
                                   ForwardedMessageDisplayStrategy display)
    : nodes(std::vector(nodes)), display(std::move(display)) {}
ForwardedMessage::ForwardedMessage(std::vector<ForwardedNode> inNodes,
                                   ForwardedMessageDisplayStrategy display)
...
static_assert(sizeof(char *) == 8,
              "Please make sure the size of pointers is 8 bytes");
static_assert(sizeof(MiraiCPString) == 3 * 8,
              "Please make sure the size of size_t is 8 bytes");
}  // namespace MiraiCP
#endif
...
LOADER_API_COUNT
void loggerInterface(const MiraiCPString &content, const MiraiCPString &name,
                     long long id, int level);
LOADER_API_COUNT
...
constexpr inline interface_funcs collect_interface_functions(bool admin) {
  ...
  if (admin) {
    ...
    constexpr size_t line0 = __LINE__;
    interface_funcs t2 = {
        pluginOperation, loggerInterface, showAllPluginId,
        pushTask,        pushTaskWithId,  timer,
    };
    constexpr size_t line1 = __LINE__;
    constexpr size_t adminapi_construct_number = line1 - line0 - 3;
    static_assert(adminapi_construct_number ==
                  interface_funcs::normal_api_count);
    return t2;
  }
}
#endif
...
void Logger::log_interface(const std::string &content, int level) {
  ...
  LibLoader::LoaderApi::loggerInterface(
      content, ("plugin/" + MiraiCP::CPPPlugin::config.getName()), -1, level);
}
void IdLogger::log_interface(const std::string &content, int level) {
  handler_trigger(content, level);
  LibLoader::LoaderApi::loggerInterface(content, "", static_cast<long long>(id),
                                        level);
}
IdLogger::IdLogger(QQID inId, Logger *l) : id(inId) {
  this->loggerhandler = l->loggerhandler;
}
void Logger_interface::handler_trigger(Logger_interface::string log,
                                       int level) {
  if (!loggerhandler) create_loggerhandler();
  if (loggerhandler->enable && loggerhandler->action)
    loggerhandler->action(std::move(log), level);
}
...
/// 远程资源出现问题
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT RemoteAssetException
    : public MiraiCPExceptionCRTP<RemoteAssetException> {
 public:
  explicit RemoteAssetException(const string &e, string _filename, int _lineNum)
      : MiraiCPExceptionCRTP(e, std::move(_filename), _lineNum) {}
...
namespace MiraiCP {
using json = nlohmann::json;
const char *const *const SingleMessage::messageType =
    SingleMessageType::messageTypeInternal + 6;
const char *const *const SingleMessage::miraiCodeName =
    SingleMessageType::miraiCodeNameInternal + 6;
SingleMessage::SingleMessage(int inType, std::string content,
...
std::string SingleMessage::toMiraiCode() const {
  if (internalType > 0)
    if (internalType == Types::At_t)
      ...
      return "[mirai:" + getTypeString(internalType) + prefix +
             Tools::escapeToMiraiCode(content) + "]";
  else
    return content;
}
bool SingleMessage::operator==(const SingleMessage &m) const {
  return this->internalType == m.internalType &&
         this->toMiraiCode() == m.toMiraiCode();
}
PlainText::PlainText(std::string inStr) noexcept
    : SingleMessage(PlainText::type(), std::move(inStr)) {}
PlainText::PlainText(const SingleMessage &sg) : SingleMessage(sg) {
  if (sg.internalType != type())
    throw IllegalArgumentException(
        "Cannot convert(" + getTypeString(sg.internalType) + ") to PlainText",
        MIRAICP_EXCEPTION_WHERE);
...
At::At(const SingleMessage &sg) : SingleMessage(sg) {
  if (sg.internalType != type())
    throw IllegalArgumentException(
        "Cannot convert(" + getTypeString(sg.internalType) + ") to At",
        MIRAICP_EXCEPTION_WHERE);
...
bool At::operator==(const At &a) const { return this->target == a.target; }
AtAll::AtAll() : SingleMessage(AtAll::type(), "", "") {}
nlohmann::json AtAll::toJson() const {
  return {{"type", SingleMessage::messageType[this->internalType]}};
}
std::string AtAll::toMiraiCode() const { return "[mirai:atall] "; }
/*图片类实现*/
void Image::refreshInfo() {
  std::string re =
      KtOperation::ktOperation(KtOperation::QueryImgInfo, toJson());
  if (re == "E1")
...
Image Image::deserialize(const std::string &str) {
  ...
  return Image(j["imageId"], j["size"], j["width"], j["height"], j["imageType"],
               j["isEmoji"]);
}
Image::Image(const std::string &imageId, size_t size, int width, int height,
             std::string type, bool isEmoji)
    : SingleMessage(Image::type(), imageId),
      id(imageId),
      imageType(std::move(type)),
      size(size),
      width(width),
      height(height),
      isEmoji(isEmoji) {}
...
bool Image::operator==(const Image &i) const { return this->id == i.id; }
...
bool Image::isUploaded(QQID botid) {
  ...
  if (this->size == 0)
    throw IllegalArgumentException("size不能为0", MIRAICP_EXCEPTION_WHERE);
  std::string re = KtOperation::ktOperation(
      KtOperation::ImageUploaded, {{"botId", botid}, {"image", toJson()}});
  return re == "true";
}
FlashImage FlashImage::deserialize(const std::string &str) {
  json j = json::parse(str);
  return FlashImage(j["imageId"], j["size"], j["width"], j["height"],
                    j["imageType"]);
}
FlashImage::FlashImage(const Image &img) {
  if (img.internalType == FlashImage::type()) {
    *this = static_cast<const FlashImage &>(img);
    ...
    MIRAICP_THROW(IllegalArgumentException,
                  "拷贝构造FlashImage时传入参数无法正确转换");
  }
}
FlashImage::FlashImage(const std::string &imageId, size_t size, int width,
...
LightApp::LightApp(std::string content)
    : SingleMessage(LightApp::type(), std::move(content)) {}
...
LightApp::LightApp(const SingleMessage &sg) : SingleMessage(sg) {
  if (sg.internalType != type())
    throw IllegalArgumentException(
        "Cannot convert(" + getTypeString(sg.internalType) + ") to LighApp",
        MIRAICP_EXCEPTION_WHERE);
}
...
ServiceMessage::ServiceMessage(const URLSharer &a)
    : SingleMessage(
          ...
          R"(<?xml version="1.0" encoding="utf-8"?><msg templateID="12345" action="web" brief=")" +
              ...
              R"("><item layout="2"><picture cover=")" + a.cover +
              "\"/><title>" + a.title + "</title><summary>" + a.summary +
...
QuoteReply::QuoteReply(MessageSource source)
    : SingleMessage(QuoteReply::type(), source.serializeToString()),
      source(std::move(source)) {}
QuoteReply::QuoteReply(const SingleMessage &m) : SingleMessage(m) {
  if (m.internalType != type())
    throw IllegalArgumentException("cannot convert type(" +
                                       std::to_string(m.internalType) +
                                       "to QuoteReply",
                                   MIRAICP_EXCEPTION_WHERE);
...
OnlineAudio::OnlineAudio(std::string f, std::array<uint8_t, 16> md5, int size,
                         int codec, int length, std::string url)
...
RemoteFile::RemoteFile(const std::string &i, unsigned int ii, std::string n,
                       long long s, std::string p, struct Dinfo d,
                       struct Finfo f)
    : SingleMessage(RemoteFile::type(), i + "," + std::to_string(ii) + "," +
                                            Tools::escapeToMiraiCode(n) + "," +
                                            std::to_string(s)),
      id(i),
      internalid(ii),
      name(std::move(n)),
...
RemoteFile::RemoteFile(const std::string &i, unsigned int ii, std::string n,
                       long long s)
    : SingleMessage(6, i + "," + std::to_string(ii) + "," +
                           Tools::escapeToMiraiCode(n) + "," +
                           std::to_string(s)),
      id(i),
      internalid(ii),
...
RemoteFile RemoteFile::deserializeFromString(const std::string &source) {
  ...
  try {
    auto re = RemoteFile(j["id"], j["internalId"], j["name"],
                         j["detailInfo"]["size"]);
    if (j.contains("downloadInfo")) {
      auto tmp = json_jsonmover(j, "downloadInfo");
      struct Dinfo d {
        json_jsonmover(tmp, "url"), json_jsonmover(tmp, "md5"),
    ...
    if (j["detailInfo"].contains("uploaderId")) {
      ...
      struct Finfo f {
        json_jsonmover(tmp, "size"), json_jsonmover(tmp, "uploaderId"),
    ...
    if (j.contains("path")) re.path = json_jsonmover(j, "path");
    return re;
...
RemoteFile RemoteFile::plus(unsigned int ii) {
  RemoteFile tmp(*this);
  tmp.internalid = ii;
  tmp.content = id + "," + std::to_string(ii) + "," +
                Tools::escapeToMiraiCode(name) + "," + std::to_string(size);
  return tmp;
}
std::string RemoteFile::serializeToString() {
  ...
  j["name"] = this->name;
  j["size"] = this->size;
  if (this->path.has_value()) j["path"] = this->path.value();
  return j.dump();
}
...
nlohmann::json Face::toJson() const {
  return {{"type", SingleMessage::messageType[this->internalType]}, {"id", id}};
}
std::string Face::toMiraiCode() const {
  return "[mirai:face:" + std::to_string(id) + "]";
}
bool Face::operator==(const Face &f) const { return this->id == f.id; }
MusicShare::MusicShare(std::string appName, std::string title,
                       std::string summary, std::string jumpUrl,
                       std::string picUrl, std::string musicUrl,
                       std::string brief)
...
std::string MusicShare::toMiraiCode() const {
  return "[mirai:musicshare:" + appName + "," + title + "," + summary + "," +
         jumpUrl + "," + picUrl + "," + musicUrl + "," + brief + "]";
}
MarketFace::MarketFace(std::array<uint8_t, 16> id)
    : SingleMessage(MarketFace::type(), ""), faceId(id) {}
...
struct IMiraiData {
 ...
 public:
  ...
  /// 获取锁
  std::shared_mutex &getMutex() { return Locker._mtx; }
...
/// 通常为Mirai返回
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT IllegalStateException
    : public MiraiCPExceptionCRTP<IllegalStateException> {
 public:
  explicit IllegalStateException(const std::string &text, string _filename,
                                 int _lineNum)
      : MiraiCPExceptionCRTP("状态异常:" + text, std::move(_filename),
                             _lineNum) {}
...
/// 文件读取异常.
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT UploadException
    : public MiraiCPExceptionCRTP<UploadException> {
 public:
  explicit UploadException(const std::string &text, string _filename,
                           int _lineNum)
      : MiraiCPExceptionCRTP("上传(图片/文件)异常" + text, std::move(_filename),
                             _lineNum) {}
...
auto GetGroupFromPool(QQID groupid, QQID botid) noexcept {
  static std::unordered_map<
      QQID, std::unordered_map<QQID, std::shared_ptr<Group::DataType>>>
      Pool;
...
auto GetGroupFromPool(const json &in_json) {
  try {
    ...
    throw IllegalArgumentException("构造Group时传入的json异常",
                                   MIRAICP_EXCEPTION_WHERE);
  }
}
Group::Group(QQID groupId, QQID botId)
    : Contact(GetGroupFromPool(groupId, botId)) {
...
Group::Group(json in_json) : Contact(GetGroupFromPool(in_json)) {
  ...
  if (in_json.contains("nickOrNameCard"))
    ActualDataPtr->_nickOrNameCard =
        json_stringmover(in_json, "nickOrNameCard");
  else
    needRefresh = true;
  if (in_json.contains("avatarUrl"))
    ActualDataPtr->_avatarUrl = json_stringmover(in_json, "avatarUrl");
  else
    needRefresh = true;
...
json Group::AnnouncementParams::serializeToJson() {
  return {{"sendToNewMember", send2new},
          {"isPinned", pinned},
          {"showEditCard", showEditCard},
          {"showPopup", showPopup},
          {"requireConfirmation", requireConfirm}};
}
Group::OnlineAnnouncement Group::OfflineAnnouncement::publishTo(
    const Group &g) {
  json j{
      {"botId", g.botid()},
      {"groupId", g.id()},
...
Group::OnlineAnnouncement Group::OnlineAnnouncement::deserializeFromJson(
    const json &j) {
  return Group::OnlineAnnouncement{
      ...
      j["imageId"],
      j["confirmationNum"],
      {j["params"]["sendToNewMember"], j["params"]["requireConfirmation"],
       j["params"]["isPinned"], j["params"]["showEditCard"],
       j["params"]["showPopup"]}};
}
...
RemoteFile Group::sendFile(const std::string &path,
                           const std::string &filepath) {
  json j{{"path", path}, {"filePath", filepath}, {"contact", toJson()}};
  auto re = KtOperation::ktOperation(KtOperation::SendFile, j);
  if (re == "E2")
    throw UploadException("找不到" + filepath + "位置:C-uploadfile",
                          MIRAICP_EXCEPTION_WHERE);
  if (re == "E3")
    throw UploadException(
        "Upload "
        "error:路径格式异常，应为'/xxx.xxx'或'/xx/"
        "xxx.xxx'目前只支持群文件和单层路径, path:" +
            path,
        MIRAICP_EXCEPTION_WHERE);
  MIRAICP_ERROR_HANDLE(re, "");
...
RemoteFile Group::getFile(const std::string &path, const std::string &id) {
  if (path.empty() || path == "/") return this->getFileById(id);
  ...
  if (re == "E2")
    throw RemoteAssetException(
        "Get error: 文件路径不存在, path:" + path + ",id:" + id,
        MIRAICP_EXCEPTION_WHERE);
  MIRAICP_ERROR_HANDLE(re, "");
  return RemoteFile::deserializeFromString(re);
}
RemoteFile Group::getFileById(const std::string &id) {
  ...
  std::string re = KtOperation::ktOperation(KtOperation::RemoteFileInfo, j);
  if (re == "E1")
    throw RemoteAssetException("Get error: 文件路径不存在, id:" + id,
                               MIRAICP_EXCEPTION_WHERE);
  MIRAICP_ERROR_HANDLE(re, "");
  return RemoteFile::deserializeFromString(re);
}
...
std::vector<Group::file_short_info> Group::getFileList(
...
Member Group::operator[](QQID a) { return getMember(a); }
IMPL_GETTER(setting)
...
std::optional<Member> Group::queryCurrentHonorMember(HonorMemberType type) {
  auto re = KtOperation::ktOperation(
      KtOperation::queryHonorMember,
      json{{"honorType", static_cast<int>(type)}, {"contact", toJson()}});
  if (re == "E1") {
    return std::nullopt;
  }
  return Member(json::parse(re));
}
QQID Group::groupId() { return GetDataInternal()->_groupId; }
...
std::string replace(std::string str, std::string_view from,
                    std::string_view to) {
  size_t start_pos = 0;
...
std::vector<std::string> split(const std::string &text,
  ...
  return {std::sregex_token_iterator(text.begin(), text.end(), ws_re, -1),
...
/// 获取群成员错误
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT MemberException
    : public MiraiCPExceptionCRTP<MemberException> {
 public:
  enum MemberExceptionType : int { OtherType, NoSuchGroup, NoSuchMember };
  MemberExceptionType type = OtherType;
  /*
   *   "1" - 找不到群
   *	"2" - 找不到群成员
   */
  explicit MemberException(int _type, string _filename, int _lineNum)
      : MiraiCPExceptionCRTP(
            [&]() -> string {
...
/// 禁言异常
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT MuteException
    : public MiraiCPExceptionCRTP<MuteException> {
 public:
  /*
   *	 禁言时间超出0s~30d
   */
  MuteException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("禁言时长不在0s~30d中间", std::move(_filename),
                             _lineNum) {}
...
auto GetMemberFromPool(QQID id, QQID groupId, QQID botid) noexcept {
  using Tools::idpair;
  static std::unordered_map<
      idpair, std::unordered_map<QQID, std::shared_ptr<Member::DataType>>>
      Pool;
...
auto GetMemberFromPool(const json &in_json) {
  try {
    return GetMemberFromPool(in_json["id"], in_json["groupId"],
                             in_json["botId"]);
  } catch (const nlohmann::detail::exception &) {
    throw IllegalArgumentException("构造Member时传入的json异常",
                                   MIRAICP_EXCEPTION_WHERE);
  }
}
...
Member::Member(nlohmann::json in_json) : Contact(GetMemberFromPool(in_json)) {
  ...
  if (in_json.contains("avatarUrl"))
    ...
    needrefresh = true;
  if (in_json.contains("anonymous"))
    ActualDataPtr->_anonymous = in_json["anonymous"].get<bool>();
  else
    needrefresh = true;
  if (needrefresh) forceRefreshNextTime();
}
void Member::mute(long long sec) const {
  std::string re = KtOperation::ktOperation(
      KtOperation::MuteM, json{{"time", sec}, {"contact", toJson()}});
  if (re == "E4") throw MuteException(MIRAICP_EXCEPTION_WHERE);
}
...
void MemberData::refreshInfo() {
  ...
  if (result == "E1") throw MemberException(1, MIRAICP_EXCEPTION_WHERE);
  if (result == "E2") throw MemberException(2, MIRAICP_EXCEPTION_WHERE);
  {
...
LowLevelAPI::info LowLevelAPI::info0(const std::string &source) {
  ...
  info re{json_stringmover(j, "nickOrNameCard"),
          json_stringmover(j, "avatarUrl")};
  return re;
}
...
nlohmann::json MessageChain::toJson() const {
  ...
  for (auto &&a : *this) j.emplace_back(a->toJson());
  return j;
}
std::string MessageChain::toString() const { return toJson().dump(); }
MessageChain MessageChain::deserializationFromMiraiCode(const std::string &m) {
  std::string re =
      KtOperation::ktOperationStr(KtOperation::DeserializeMiraiCode, m);
  return MessageChain::deserializationFromMessageJson(json::parse(re));
}
MessageChain MessageChain::deserializationFromMessageSourceJson(json j,
                                                                bool origin) {
  if (origin) j = j["originalMessage"];
  MessageChain mc;
  if (j.empty()) return mc;
  if (!j.is_array())
    throw IllegalArgumentException(
        std::string(__func__) + "输入的json应当是数组类型",
        MIRAICP_EXCEPTION_WHERE);
  ...
  if (jArray[0]["type"] == "MessageOrigin") {
    if (jArray[0]["kind"] == "MUSIC_SHARE") {
      if (jArray.size() < 2)
        throw IllegalArgumentException(
            std::string(__func__) + "数组长度应至少为2",
            MIRAICP_EXCEPTION_WHERE);
      mc.add(MusicShare(jArray[1]["kind"], jArray[1]["title"],
                        jArray[1]["summary"], jArray[1]["jumpUrl"],
                        jArray[1]["pictureUrl"], jArray[1]["musicUrl"],
                        jArray[1]["brief"]));
      return mc;
    }
    return mc;
  ...
  for (auto &node : jArray) {
    ...
    if (node["type"] == "OnlineAudio") {
      mc.add(OnlineAudio(node["filename"], node["fileMd5"], node["fileSize"],
                         node["codec"], node["length"],
                         node["urlForDownload"]));
      continue;
    }
    if (node["type"] == "FileMessage") {
      mc.add(RemoteFile(node["id"], node["internalId"], node["name"],
                        node["size"]));
      continue;
    }
    ...
    switch (SingleMessage::getKey(node["type"])) {
      case -2:
        mc.add(QuoteReply(
            MessageSource::deserializeFromString(node["source"].dump())));
        break;
      case 0:
      ...
      default:
        Logger::logger.warning(
            "MiraiCP碰到了意料之中的错误(原因:"
            "接受到的SimpleMessage在MessageSource解析支持之外)\n请到MiraiCP("
            "github.com/Nambers/"
            "MiraiCP)发送issue并复制本段信息使MiraiCP可以支持这种消息: node:" +
            node.dump());
  ...
  for (auto &node : j.get<json::array_t>()) {
    switch (SingleMessage::getKey(node["type"])) {
      case SingleMessageType::MusicShare_t:
        mc.add(MusicShare(node["kind"], node["title"], node["summary"],
                          node["jumpUrl"], node["pictureUrl"], node["musicUrl"],
                          node["brief"]));
        break;
      ...
      case SingleMessageType::Image_t:
        mc.add(Image(node["imageId"], node["size"].get<size_t>(),
                     node["width"].get<int>(), node["height"].get<int>(),
                     node["imageType"], node["isEmoji"].get<bool>()));
        break;
...
std::vector<std::string> MessageChain::toMiraiCodeVector() const {
  ...
  for (auto &&a : *this) tmp.emplace_back(a->toMiraiCode());
  return tmp;
}
...
MessageChain MessageChain::deserializationFromMessageSourceJson(
    const std::string &msg, bool origin) {
  return deserializationFromMessageSourceJson(nlohmann::json::parse(msg),
                                              origin);
}
internal::Message::Message(internal::Message::Super msgptr) noexcept
    : Super(std::move(msgptr)) {}
int internal::Message::getType() const { return (*this)->internalType; }
std::string internal::Message::toMiraiCode() const {
  return (*this)->toMiraiCode();
}
std::string internal::Message::toJson() const { return (*this)->toJson(); }
...
void internal::Message::messageThrow(const std::string &from,
                                     const std::string &to, const char *file,
                                     int line) {
  throw IllegalArgumentException("cannot convert from " + from + " to " + to,
...
class MiraiCPThreadException
    : public MiraiCPExceptionCRTP<MiraiCPThreadException> {
 public:
  /// 抛出异常的线程 ID
  std::thread::id threadId;

 public:
  explicit MiraiCPThreadException(const std::string &exception_content,
                                  std::thread::id threadId, string _filename,
                                  int _lineNum)
      : MiraiCPExceptionCRTP(
            exception_content + " at threadId: " + getThreadIdStr(threadId),
            std::move(_filename), _lineNum),
        threadId(threadId) {}
...
void MiraiCPNewThread::threadThrows(const std::string &content) {
  MiraiCPThreadException exNew(content, std::this_thread::get_id(),
                               MIRAICP_EXCEPTION_WHERE);
  exNew.raise();
...
MessageSource Contact::sendVoiceImpl(std::string path) const {
  json j{{"path", std::move(path)}, {"contact", toJson()}};
...
MessageSource Contact::quoteAndSendImpl(std::string msg,
                                        const MessageSource &ms) const {
...
MessageSource Contact::sendMsgImpl(std::string msg) const {
  if (msg.empty()) {
    throw IllegalArgumentException("不能发送空信息, 位置: Contact::SendMsg",
                                   MIRAICP_EXCEPTION_WHERE);
  }
  nlohmann::json j{{"message", std::move(msg)}, {"contact", toJson()}};
  auto re = KtOperation::ktOperation(
      KtOperation::Send, j, true, "reach a error area, Contact::sendMsgImpl");
  return MessageSource::deserializeFromString(re);
}
void Contact::SetInternalData(std::shared_ptr<IContactData> Data) {
...
void Contact::refreshInfo() { InternalData->requestRefresh(); }
void Contact::forceRefreshNextTime() { InternalData->forceRefreshNextTime(); }
...
MessageSource Contact::unpackMsg(const MiraiCodeable &msg) const {
  return sendMsgImpl(
      MessageChain::deserializationFromMiraiCode(msg.toMiraiCode()).toString());
}
MessageSource Contact::unpackMsg(std::string msg) const {
  return sendMsgImpl(MessageChain(PlainText(std::move(msg))).toString());
}
MessageSource Contact::unpackQuote(const SingleMessage &s,
                                   const MessageSource &ms) const {
  return quoteAndSendImpl(MessageChain(s).toString(), ms);
}
MessageSource Contact::unpackQuote(const std::string &s,
                                   const MessageSource &ms) const {
  return quoteAndSendImpl(s, ms);
}
MessageSource Contact::unpackQuote(const MessageChain &mc,
                                   const MessageSource &ms) const {
...
auto GetFriendPool(QQID id, QQID botid) noexcept {
  static std::unordered_map<
      QQID, std::unordered_map<QQID, std::shared_ptr<Friend::DataType>>>
      Pool;
...
auto GetFriendPool(const json &in_json) {
  try {
    ...
    throw IllegalArgumentException("构造Friend时传入的json异常",
                                   MIRAICP_EXCEPTION_WHERE);
  }
}
/*好友类实现*/
...
void Friend::sendNudge() {
  std::string re = KtOperation::ktOperation(KtOperation::SendNudge, toJson());
  if (re == "E1")
    throw IllegalStateException("发送戳一戳失败，登录协议不为phone/ipad",
                                MIRAICP_EXCEPTION_WHERE);
}
}  // namespace MiraiCP
/// Friend.cpp END
...
namespace MiraiCP::KtOperation {
std::string ktOperation(operation_set type, const nlohmann::json &data,
                        bool catchErr, const std::string &errorInfo) {
...
namespace LibLoader::LoaderApi {
const interface_funcs *get_loader_apis();
MIRAICP_EXPORT void set_loader_apis(
    const LibLoader::LoaderApi::interface_funcs *apis) noexcept;
...
extern "C" {
/// 插件开启入口
MIRAICP_EXPORT int FUNC_ENTRANCE(
    const LibLoader::LoaderApi::interface_funcs &funcs) {
  static_assert(std::is_same_v<decltype(&FUNC_ENTRANCE),
                               LibLoader::plugin_entrance_func_ptr>);
  ...
  try {
    ...
    try {
      std::cerr.flush();
      e.raise();
      Logger::logger.error("插件(id=" + CPPPlugin::config.getId() +
                           ", name=" + CPPPlugin::config.name + ")启动失败");
    } catch (...) {
    }
    return PLUGIN_ERROR;
...
MIRAICP_EXPORT int FUNC_EXIT() {
  static_assert(
      std::is_same_v<decltype(&FUNC_EXIT), LibLoader::plugin_func_ptr>);
  using namespace MiraiCP;
  if (LibLoader::LoaderApi::get_loader_apis() == nullptr) return PLUGIN_NORMAL;
  MIRAICP_CRITICAL_NOEXCEPT_BLOCK(Logger::logger.info(
      "开始禁用插件：" + MiraiCP::CPPPlugin::config.getId());)
  Event::clear();
  MIRAICP_CRITICAL_NOEXCEPT_BLOCK(if (CPPPlugin::plugin != nullptr)
...
MIRAICP_EXPORT int FUNC_EVENT(const MiraiCP::MiraiCPString &c) {
  static_assert(
      std::is_same_v<decltype(&FUNC_EVENT), LibLoader::plugin_event_func_ptr>);
  using namespace MiraiCP;
  ...
  try {
    ...
    MIRAICP_CRITICAL_NOEXCEPT_BLOCK(
  ...
  try {
    if (!j.is_object() || !j.contains("eventId") || !j["eventId"].is_number()) {
      MIRAICP_CRITICAL_NOEXCEPT_BLOCK(Logger::logger.error(
          "Json格式错误：没有eventId这一field或这一field不是数字类型");)
      return PLUGIN_NORMAL;
    }
    type = j["eventId"].get<int>();
    if (type != eventTypes::Command && Event::noRegistered(type))
      return PLUGIN_NORMAL;
  } catch (...) {
    MIRAICP_CRITICAL_NOEXCEPT_BLOCK(
        Logger::logger.error("Json格式错误：解析json时遇到无法预料的异常");)
    return PLUGIN_NORMAL;
  }
  try {
    ...
    MIRAICP_CRITICAL_NOEXCEPT_BLOCK(
        Logger::logger.error("json格式化异常,位置：FUNC_EVENT");
        Logger::logger.error(e.what()); Logger::logger.error(content);)
    return PLUGIN_NORMAL;
...
/// 获取 Plugin Info
/// 如果未正确定义，插件无法正确加载
/// dev: 该函数不可调用loader api；因为会在入口函数调用前先调用，loader
/// api未初始化
MIRAICP_EXPORT const MiraiCP::PluginConfig *PLUGIN_INFO() {
  static_assert(
      std::is_same_v<decltype(&PLUGIN_INFO), LibLoader::plugin_info_func_ptr>);
...
/// 撤回异常
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT RecallException
    : public MiraiCPExceptionCRTP<RecallException> {
 public:
  RecallException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("该消息已经被撤回", std::move(_filename),
                             _lineNum) {}
  static string exceptionType() { return "RecallException"; }
};
void MessageSource::recall() const {
  std::string re =
      KtOperation::ktOperationStr(KtOperation::Recall, serializeToString());
  if (re == "E2") throw RecallException(MIRAICP_EXCEPTION_WHERE);
}
MessageSource::MessageSource(std::string ids, std::string internalids,
                             std::string source)
    : ids(std::move(ids)),
      internalids(std::move(internalids)),
      source(std::move(source)) {}
std::string MessageSource::serializeToString() const { return source; }
MessageSource MessageSource::deserializeFromString(const std::string &source) {
  ...
  try {
    ...
    Logger::logger.error(
        "消息源序列化出错，格式不符合(MessageSource::deserializeFromString)");
    Logger::logger.error(source);
    Logger::logger.error(e.what());
    throw IllegalArgumentException(
        std::string("消息源序列化出错，格式不符合(MessageSource::"
...
void IMiraiData::forceRefreshNextTime() { Locker._inited = false; }
std::string IMiraiData::toString() const { return toJson().dump(); }
...
/// 超时
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT TimeOutException
    : public MiraiCPExceptionCRTP<TimeOutException> {
 public:
  explicit TimeOutException(const std::string &e, string _filename,
                            int _lineNum)
      : MiraiCPExceptionCRTP(e, std::move(_filename), _lineNum) {}
...
GroupMessageEvent::GroupMessageEvent(BaseEventData j)
    : BotEvent(j.botId),
...
MessageChain GroupMessageEvent::nextMessage(long time, bool halt) const {
  json j{{"contact", this->group.toJson()}, {"time", time}, {"halt", halt}};
  ...
  return MessageChain::deserializationFromMessageJson(
             nlohmann::json::parse(json_stringmover(re, "message")))
...
MessageChain GroupMessageEvent::senderNextMessage(long time, bool halt) const {
  json j{{"contact", this->sender.toJson()}, {"time", time}, {"halt", halt}};
  std::string r = KtOperation::ktOperation(KtOperation::NextMsg, j);
  if (r == "E1")
    throw TimeOutException("取下一条信息超时", MIRAICP_EXCEPTION_WHERE);
  json re = json::parse(r);
...
PrivateMessageEvent::PrivateMessageEvent(BaseEventData j)
    : BotEvent(j.botId),
...
GroupInviteEvent::GroupInviteEvent(BaseEventData j)
    : BotEvent(j.botId),
      source(json_stringmover(j.eventData, "request")),
      inviterNick(json_stringmover(j.eventData, "invitorNick")),
...
void groupInviteInternal(const std::string &source, QQID botid, bool accept) {
  nlohmann::json j{{"source", source}, {"sign", accept}, {"botId", botid}};
  std::string re = KtOperation::ktOperation(KtOperation::Gioperation, j);
  if (re == "E")
    Logger::logger.error("群聊邀请事件同意失败(可能因为重复处理),id:" + source);
}
void GroupInviteEvent::reject() {
  groupInviteInternal(source, bot.id(), false);
}
void GroupInviteEvent::accept() { groupInviteInternal(source, bot.id(), true); }
NewFriendRequestEvent::NewFriendRequestEvent(BaseEventData j)
    : BotEvent(j.botId),
      source(json_stringmover(j.eventData, "request")),
      from(j.object->id, j.object->botId),
      fromGroup(j.subject == std::nullopt
                    ? std::nullopt
                    : std::optional(Group(j.subject->id, j.subject->botId))),
      nick(json_stringmover(j.eventData, "requesterNick")),
      message(json_stringmover(j.eventData, "message")),
      requestEventId(j.eventData["requestEventId"]) {}
void newFriendRequestInternal(const std::string &source, QQID botId,
                              bool accept, bool ban = false) {
  nlohmann::json j{
      {"source", source}, {"sign", accept}, {"botId", botId}, {"ban", ban}};
  std::string re = KtOperation::ktOperation(KtOperation::Nfroperation, j);
  if (re == "E")
    Logger::logger.error("好友申请事件同意失败(可能因为重复处理),id:" + source);
}
...
MemberJoinEvent::MemberJoinEvent(BaseEventData j)
    : BotEvent(j.botId),
      type(joinType(j.eventData["eventType"].get<int>())),
      member(j.object->id, j.subject->groupId, j.botId),
      group(j.subject->id, j.subject->botId),
      inviter(j.object == std::nullopt
                  ? std::nullopt
                  : std::optional(Member(j.object->id, j.object->groupId,
                                         j.object->botId))) {}
MemberLeaveEvent::MemberLeaveEvent(BaseEventData j)
    : BotEvent(j.botId),
      member(j.object->id, j.object->groupId, j.object->botId),
      group(j.subject->id, j.subject->botId),
      operater(!j.eventData.contains("operator")
                   ? std::nullopt
                   : std::optional(Member(j.eventData["operator"]["id"],
                                          j.eventData["operator"]["groupId"],
                                          j.eventData["operator"]["botId"]))),
      type(j.eventData["eventType"]) {}
FriendRecallEvent::FriendRecallEvent(BaseEventData j)
    : BotEvent(j.botId),
...
MemberRecallEvent::MemberRecallEvent(BaseEventData j)
    : BotEvent(j.subject->botId),
      time(j.eventData["messageTime"]),
      author(j.eventData["author"]["id"], j.eventData["author"]["groupId"],
             j.eventData["author"]["botId"]),
      operater(j.object->id, j.object->groupId, j.object->botId),
      ids(j.eventData["messageIds"].dump()),
      internalIds(j.eventData["messageInternalIds"].dump()) {}
BotJoinGroupEvent::BotJoinGroupEvent(BaseEventData j)
    : BotEvent(j.botId),
      group(j.subject->id, j.subject->botId),
      inviter(j.eventData.contains("inviter")
                  ? std::optional(Member(j.eventData["inviter"]["id"],
                                         j.eventData["inviter"]["groupId"],
                                         j.eventData["inviter"]["botId"]))
                  : std::nullopt),
      type(j.eventData["eventType"]) {}
GroupTempMessageEvent::GroupTempMessageEvent(BaseEventData j)
    : BotEvent(j.botId),
      group(j.subject->id, j.subject->botId),
      ...
      message(MessageChain::deserializationFromMessageJson(
                  ...
                  .plus(MessageSource::deserializeFromString(
                      j.eventData["source"]))) {}
NudgeEvent::NudgeEvent(BaseEventData j)
    : BotEvent(j.botId),
      from(j.object->toContactPointer()),
      target(BaseEventData::BuiltInContact(j.eventData["target"])
                 .toContactPointer()),
      subject(j.subject->toContactPointer()) {}
BotLeaveEvent::BotLeaveEvent(BaseEventData j)
    : BotEvent(j.botId),
      group(j.subject->id, j.subject->botId),
      type(j.eventData["eventType"]),
      operater(j.object->id, j.object->groupId, j.object->botId) {}
MemberJoinRequestEvent::MemberJoinRequestEvent(BaseEventData j)
    : BotEvent(j.botId),
      source(json_jsonmover(j.eventData, "requestData")),
      group(j.subject->id, j.subject->botId),
      inviter(
          j.eventData.contains("inviter")
              ? std::optional(Member(json_jsonmover(j.eventData, "inviter")))
              : std::nullopt),
      from(j.object->id, j.object->groupId, j.object->botId),
      fromNick(json_jsonmover(j.eventData, "fromNick")),
      message(json_jsonmover(j.eventData, "message")) {}
/**
 * @brief 底层通过MemberJoinRequest
 * @param s 序列化后的文本
 */
void memberJoinRequestInternal(std::string_view s, QQID botid, bool sign,
                               std::string msg = "") {
  nlohmann::json j{
      {"source", s}, {"botId", botid}, {"sign", sign}, {"msg", std::move(msg)}};
...
MessagePreSendEvent::MessagePreSendEvent(BaseEventData j)
    : BotEvent(j.botId),
      target(j.subject->toContactPointer()),
      message(MessageChain::deserializationFromMessageJson(
          nlohmann::json::parse(json_stringmover(j.eventData, "message")))) {}
  ...
  switch (type) {
    ...
    case eventTypes::Command: {
      CommandManager::commandList[j.eventData["bindId"]]->onCommand(
          j.eventData.contains("contact")
              ? Contact::deserializeToPointer(
                    json_jsonmover(j.eventData, "contact"))
              : nullptr,
          Bot(j.botId),
          MessageChain::deserializationFromMessageJson(
              json::parse(json_stringmover(j.eventData, "message"))));
      break;
    }
    default: {
...
BaseEventData::BuiltInContact::BuiltInContact(nlohmann::json in_json) {
  ...
  this->id = in_json["id"];
  if (in_json.contains("groupId")) this->groupId = in_json["groupId"];
...
std::shared_ptr<Contact> BaseEventData::BuiltInContact::toContactPointer() {
  switch (this->type) {
    ...
    default:
      throw APIException("Type of builtInContact doesn't match or implement",
                         MIRAICP_EXCEPTION_WHERE);
  }
}
...
bool NodeHandle::isEnable() const { return handle->flag.load(); }
void NodeHandle::stop() { handle->flag.store(false); }
void NodeHandle::resume() { handle->flag.store(true); }
NodeHandle::~NodeHandle() { delete handle; }
...
Event::eventNode::eventNode(std::function<bool(MiraiCPEvent *)> f)
    : func(std::move(f)), _handle(true) {}
}  // namespace MiraiCP
/// Event.cpp END
...
void push_task(size_t id, functionPointerType func) {
  ...
  if (!insert) {
    Logger::logger.error("Fatal: task id " + std::to_string(id) +
                         " is already taken by another task!");
    return;
  }
...
void remove_task(size_t id) {
  ...
  if (0 == erased) {
    Logger::logger.error("Invalid task id " + std::to_string(id) +
                         " is requested to erase");
  }
}
/// @note dev: we can't send a std::function object to libLoader,
///  so one appropriate way is that store the function in a data structure and
///  call it in a wrapper function.
void task_interface(size_t id) {
  static_assert(std::is_same_v<LibLoader::LoaderApi::task_func_with_id,
                               decltype(&task_interface)>);
...
void raw_push_task(void (*func)()) { LibLoader::LoaderApi::pushTask(func); }
void logerror(const std::string &content) { Logger::logger.error(content); }
}  // namespace MiraiCP::ThreadTask::internal
/// ThreadTask.cpp END
/// Bot.cpp START
...
Group Bot::getGroup(QQID groupid) const { return {groupid, InternalData->_id}; }
Friend Bot::getFriend(QQID i) const { return {i, InternalData->_id}; }
std::vector<QQID> queryList(nlohmann::json bot,
                            KtOperation::QueryBotListCode type) {
  nlohmann::json j{{"contact", std::move(bot)}, {"type", type}};
...
bool Bot::operator==(const Bot &b) const { return id() == b.id(); }
}  // namespace MiraiCP
/// Bot.cpp END
/// loaderApi.cpp START
...
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT PluginNotAuthorizedException
    : public MiraiCPExceptionCRTP<PluginNotAuthorizedException> {
 public:
  explicit PluginNotAuthorizedException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP(
            "插件" + CPPPlugin::config.getId() + "没有管理权限",
            std::move(_filename), _lineNum) {}
...
/// 否则可能导致异常处理时再次抛出异常
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT PluginNotEnabledException
    : public MiraiCPExceptionCRTP<PluginNotEnabledException> {
 public:
  explicit PluginNotEnabledException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("插件" + CPPPlugin::config.getId() + "未加载",
                             std::move(_filename), _lineNum) {}
...
MIRAICP_EXPORT void reset_loader_apis() noexcept { loader_apis = nullptr; }
/// 这个函数是给本cpp以外的文件使用的，大概率用不到
const interface_funcs *get_loader_apis() { return loader_apis; }
...
void loggerInterface(const std::string &content, const std::string &name,
                     long long int id, int level) {
  LibLoader::LoaderApi::loggerInterface(content, name, id, level);
}
std::vector<std::string> showAllPluginId() {
  nlohmann::json::array_t PluginIdList =
      nlohmann::json::parse(LibLoader::LoaderApi::showAllPluginId().toString());
...
void enableAllPlugins() { LibLoader::LoaderApi::enableAllPlugins(); }
void disableAllPlugins() { LibLoader::LoaderApi::disableAllPlugins(); }
...
class MIRAICP_EXPORT BotException : public MiraiCPExceptionCRTP<BotException> {
 public:
  explicit BotException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("没有权限执行该操作", std::move(_filename),
                             _lineNum) {}
  explicit BotException(const string &d, string _filename, int _lineNum)
      : MiraiCPExceptionCRTP(d, std::move(_filename), _lineNum) {}
  static string exceptionType() { return "BotException"; }
};
/// 被禁言异常, 通常发生于发送信息
class MIRAICP_EXPORT BotIsBeingMutedException
    : public MiraiCPExceptionCRTP<BotIsBeingMutedException> {
 public:
  /// 剩余禁言时间, 单位秒
  int timeRemain;

 public:
  explicit BotIsBeingMutedException(int t, string _filename, int _lineNum)
      : MiraiCPExceptionCRTP(
            "发送信息失败, bot已被禁言, 剩余时间" + std::to_string(t),
            std::move(_filename), _lineNum),
        timeRemain(t) {}
...
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT EventCancelledException
    : public MiraiCPExceptionCRTP<EventCancelledException> {
 public:
  explicit EventCancelledException(const string &msg, string _filename,
                                   int _lineNum)
      : MiraiCPExceptionCRTP(msg, std::move(_filename), _lineNum) {}
  static string exceptionType() { return "EventCancelledException"; }
};
/// 获取群成员错误
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT FriendException
    : public MiraiCPExceptionCRTP<FriendException> {
 public:
  /*
   *   找不到好友
   */
  FriendException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("找不到好友", std::move(_filename), _lineNum) {}
  static string exceptionType() { return "FriendException"; }
};
/// 获取群错误
/// @see MiraiCPExceptionBase
class MIRAICP_EXPORT GroupException
    : public MiraiCPExceptionCRTP<GroupException> {
 public:
  GroupException(string _filename, int _lineNum)
      : MiraiCPExceptionCRTP("找不到群", std::move(_filename), _lineNum) {}
...
void ErrorHandle0(const std::string &name, int line, const std::string &re,
                  const std::string &ErrorMsg) {
  if (re == "EF") throw FriendException(name, line);
  if (re == "EG") throw GroupException(name, line);
  if (re == "EM") throw MemberException(1, name, line);
  if (re == "EMM") throw MemberException(2, name, line);
  if (re == "EB") throw BotException("找不到bot:" + re, name, line);
  if (re == "EA") throw APIException(ErrorMsg, name, line);
  if (re == "EC") throw EventCancelledException("发送信息被取消", name, line);
  if (re == "ET") throw TimeOutException("发送信息超时", name, line);
  if (re == "EP") throw BotException(name, line);
  if (re.rfind("EBM", 0) == 0)
...
MiraiCPString::MiraiCPString(MiraiCPString &&temp) noexcept { swap(temp); }
...
bool MiraiCPString::operator==(const MiraiCPString &another) const {
  return another._size == _size &&
         (_size == 0 || strcmp(another.str, str) == 0);
}
...
class OString : public std::ostream {
 ...
 public:
  ...
  void setRecorder(Recorder recorder) { sbuf.recorder = recorder; }
};
...
void MiraiCP::Redirector::setRedirectedObjs(std::ostream *outStream,
                                            std::ostream *errStream) {
  RedirectedOstreamPointers = {outStream, errStream};
}
void MiraiCP::Redirector::start() {
  outRedirector = std::make_unique<OStreamRedirector>(
      RedirectedOstreamPointers.out, outTarget.rdbuf());
  errRedirector = std::make_unique<OStreamRedirector>(
      RedirectedOstreamPointers.err, errTarget.rdbuf());
}
namespace MiraiCP::Redirector {
